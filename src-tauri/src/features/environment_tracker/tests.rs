...
use super::model::{
    ...
    EnvironmentSourceCandidateQuery, EnvironmentSourceOrigin, EnvironmentSourceParseStatus,
};
...
#[tokio::test]
async fn custom_sources_persist_filename_like_keys_without_values_and_stay_environment_scoped() {
    let context = TestContext::new().await;
    let production_id = context.environment("Production").await;
    let staging_id = context.environment("Staging").await;
    ...
    let created = context
        ...
        .create_custom_source(CreateCustomEnvironmentSource {
            ...
            name: "Firebase Credentials".to_owned(),
            key_names: vec![
                "devventory-firebase-adminsdk.json".to_owned(),
                "signing-key.p12".to_owned(),
            ],
        })
        .await
        .expect("custom source creation");

    assert_eq!(created.name, "Firebase Credentials");
    assert_eq!(created.keys.len(), 2);
    assert!(created
        .keys
        .iter()
        .any(|key| key.name == "devventory-firebase-adminsdk.json"));
    assert_eq!(
        context
            .service
            .list_custom_sources(context.project_id, production_id)
            .await
            .expect("production custom sources")
            .len(),
        1
    );
    assert!(context
        .service
        .list_custom_sources(context.project_id, staging_id)
        .await
        .expect("staging custom sources")
        .is_empty());

    let columns: Vec<String> =
        query_scalar("SELECT name FROM pragma_table_info('custom_environment_keys') ORDER BY name")
            .fetch_all(&context.pool)
            .await
            .expect("custom key schema columns");
    assert!(!columns.iter().any(|column| column.contains("value")));
    assert!(!columns.iter().any(|column| column.contains("content")));
}

#[tokio::test]
async fn custom_keys_share_matrix_identity_and_duplicate_semantics_with_file_definitions() {
    let context = TestContext::new().await;
    context.write_source("config/production.env", b"SERVICE_ACCOUNT_JSON=tracked\n");
    let production_id = context.environment("Production").await;
    let staging_id = context.environment("Staging").await;
    context
        .service
    ...
    context
        ...
        .create_custom_source(CreateCustomEnvironmentSource {
            ...
            environment_id: production_id,
            name: "Firebase Credentials".to_owned(),
            key_names: vec!["SERVICE_ACCOUNT_JSON".to_owned()],
        })
        .await
        .expect("production custom source");
    context
        .service
        .create_custom_source(CreateCustomEnvironmentSource {
            project_id: context.project_id,
    ...
    let matrix = context
        ...
        .matrix(EnvironmentMatrixQuery {
            project_id: context.project_id,
            environment_id: None,
            search: Some("service_account".to_owned()),
            page: 1,
            page_size: 25,
        })
        .await
        .expect("custom matrix");

    assert_eq!(matrix.total_items, 1);
    let row = matrix.rows.first().expect("one logical key row");
    let production_index = matrix
        .environments
        .iter()
        .position(|environment| environment.id == production_id)
        .expect("production column");
    let staging_index = matrix
        .environments
        .iter()
        .position(|environment| environment.id == staging_id)
        .expect("staging column");
    assert_eq!(
        row.cells[production_index].state,
        EnvironmentMatrixCellState::Duplicate
    );
    assert_eq!(
        row.cells[staging_index].state,
        EnvironmentMatrixCellState::Present
    );
    assert!(row.cells[production_index]
        .source_details
        .iter()
        .any(|detail| detail.origin == EnvironmentSourceOrigin::Custom
            && detail.relative_path.is_none()
            && detail.line_number.is_none()));
}

#[tokio::test]
async fn copying_custom_keys_and_sources_creates_independent_target_records() {
    let context = TestContext::new().await;
    let production_id = context.environment("Production").await;
    let staging_id = context.environment("Staging").await;
    let production = context
        ...
        .create_custom_source(CreateCustomEnvironmentSource {
            ...
            environment_id: production_id,
            name: "Firebase Credentials".to_owned(),
            key_names: vec!["google-services.json".to_owned()],
        })
        .await
    ...
    let staging = context
        ...
        .create_custom_source(CreateCustomEnvironmentSource {
            project_id: context.project_id,
            environment_id: staging_id,
            name: "Deployment Dependencies".to_owned(),
            key_names: vec![],
        })
        .await
        .expect("staging source");

    let copied_key = context
        .service
        .copy_custom_key(CopyCustomEnvironmentKey {
            project_id: context.project_id,
            key_id: production.keys[0].id,
            target_environment_id: staging_id,
            target_source_id: staging.id,
        })
        .await
        .expect("copied key");
    assert_ne!(copied_key.id, production.keys[0].id);

    let copied_source = context
        .service
        .copy_custom_source(CopyCustomEnvironmentSource {
            project_id: context.project_id,
            source_id: production.id,
            target_environment_id: staging_id,
            target_name: Some("Firebase Credentials (copy)".to_owned()),
        })
        .await
        .expect("copied source");
    assert_ne!(copied_source.id, production.id);
    assert_ne!(copied_source.keys[0].id, production.keys[0].id);
    ...
    context
        ...
        .rename_custom_source(
            ...
            production_id,
            production.id,
            "Production Firebase".to_owned(),
        )
        .await
        .expect("source rename");
    let staging_sources = context
        .service
        .list_custom_sources(context.project_id, staging_id)
        .await
        .expect("staging sources");
    assert!(staging_sources
        .iter()
        .any(|source| source.name == "Firebase Credentials (copy)"));
}

#[tokio::test]
async fn deleting_a_custom_source_cascades_keys_and_removes_orphaned_definitions() {
    let context = TestContext::new().await;
    ...
    let source = context
        ...
        .create_custom_source(CreateCustomEnvironmentSource {
            ...
            key_names: vec!["signing-key.p12".to_owned()],
        })
        .await
    ...
    context
        .service
        .delete_custom_source(context.project_id, environment_id, source.id)
        .await
        .expect("custom source deletion");

    let keys: i64 =
        query_scalar("SELECT COUNT(*) FROM custom_environment_keys WHERE source_id = ?")
            .bind(source.id.to_string())
            .fetch_one(&context.pool)
            .await
            .expect("key count");
    let definitions: i64 = query_scalar(
        "SELECT COUNT(*) FROM environment_key_definitions WHERE normalized_name = 'SIGNING-KEY.P12'",
    )
    .fetch_one(&context.pool)
    .await
